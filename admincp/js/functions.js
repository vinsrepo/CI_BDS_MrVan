...
$(document).ready(function() {
    ...
    function changeHeight(){
        ...
        CurrentHeightWindow=$("#leftside").height();
        
        HeightFrame=$(document).height(); 
        
        if(HeightFrame>CurrentHeightWindow){
                
        }else{
             $("#frameID").height(CurrentHeightWindow);
        }
        
        
        
            s = $(window).scrollTop();
            HeightLeftCurent=$("#leftside").height();
            fixTop = HeightLeftCurent-HeightWindow;
            
             if (fixTop>0&&fixTop <= s) 
             {
                $("#leftside").css({ position: "fixed", top: -fixTop-10 + "px" });
             } 
             else 
             {
                if (fixTop<=0) 
                {
                $("#leftside").css({ position: "fixed", top: 0 + "px" });
                } 
                else 
                {
                $("#leftside").css({ position: "", top: "" });
                } 
             }      
    }
	...
	$("#nav > li > a").click(function() {
       ...
       $("#frameID").height($(document).height()+5);
    });
    ...
    $("#frameID").load(function() {
        ...
        CurrentHeightWindow=$("#leftside").height();
        HeightFrame=this.contentDocument.body.scrollHeight; 
        
        if(HeightFrame>CurrentHeightWindow){
            Height=HeightFrame;
        }else{
            Height=CurrentHeightWindow;
        }
