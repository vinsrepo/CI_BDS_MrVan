...
class CKFinder_Connector_CommandHandler_Thumbnail extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        //
        $arr_thumbs=array('400x250','250x200','120x120');
        foreach($arr_thumbs as $arr_thumb){
            $size_thum=explode('x',$arr_thumb);
            $dir_400=$this->_currentFolder->getThumbsServerPath()."$arr_thumb/";
            $thumbFilePath_400 = CKFinder_Connector_Utils_FileSystem::combinePaths($dir_400, $fileName);
            if (!file_exists($dir_400)) {
                mkdir($dir_400, 0777, true);
            }

        // If the thumbnail file doesn't exists, create it now.
            if (!file_exists($thumbFilePath_400)) {
                     if(!$this->createThumb($sourceFilePath, $thumbFilePath_400, $size_thum[0], $size_thum[1], 90, true, $_thumbnails->getBmpSupported())) {
                       $this->_errorHandler->throwError(CKFINDER_CONNECTOR_ERROR_ACCESS_DENIED);
                     }
            }
        }
