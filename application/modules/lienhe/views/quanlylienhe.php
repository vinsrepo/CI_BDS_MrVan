                                ...
                                <div class="mail_content">
                                    <div class="row" style="margin-bottom: 10px;">
                                        <div class="col-md-7 col-sm-7 col-xs-7">
                                            <h3 class="mail_head"><?php echo $this->lang->line('title_quanlylienhe');?> <sup><?php echo ($totalrow!=''||$totalrow!=0?"($totalrow)":'');?></sup>
                                            </h3>                                              
                                        </div>

                                        <div class="col-md-5 col-sm-5 col-xs-5">
                                            <div class="dataTables_info pull-right" id="example2_info" role="status" aria-live="polite">
                                                Hiển thị từ <?php echo $start;?> đến <?php echo $end;?> - Tổng  <strong><?php echo $totalrow;?></strong> tin
                                            </div>
                                        ...
                                        <div class="col-md-12 col-sm-12 col-xs-12 mail_list">
                                            <table class="table table-striped table-hover">
                                                <?php if($data){ 
                                                    ...
                                                    { 
                                                        if(strlen($member['NoiDung'])>70){
                                                            $NoiDung=strip_tags(html_entity_decode($member['NoiDung']));
                                                              $NoiDung=''.substr($NoiDung,0,70).'';
                                                              $NoiDung=substr($NoiDung, 0, strrpos($NoiDung, ' ')).'...';
                                                        }else{
                                                            $NoiDung=$member['NoiDung'];
                                                        }
                                                        echo ' <tr class="unread" data-id="'.$member['IDBaiViet'].'">
                                                            <td class=""><input class=" skin-square-green" type="checkbox" name="XoaDuLieu[]" value="'.$member['IDBaiViet'].'" id="Xoa'.$member['IDBaiViet'].'"></td>
                                                            <td class=""><div class="star"><i class="fa fa-star icon-xs icon-orange"></i></div></td>
                                                            <td class="open-view">'.$member['NguoiGui'].'</td>
                                                            <td class="open-view"><span class="msgtext">'.$NoiDung.'</span></td>
                                                            <td class="open-view"><span class="msg_time">'.date("H:i d-m-Y", strtotime($member['NgayGui'])).'</span></td>
                                                            <td style="width:20px">
                                                                <a href="javascript://" class="btn btn-danger btn-sm" onclick="Delete('.$member['IDBaiViet'].');"><i class="fa fa-trash-o"></i> Xóa</a>
                                                            </td>   
                                                        </tr> 
                                                        ';
                                                    }
